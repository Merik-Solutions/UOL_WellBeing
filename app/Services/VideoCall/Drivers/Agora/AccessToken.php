...
namespace App\Services\VideoCall\Drivers\Agora;
...
class Message
{
    ...
    public function packContent()
    {
        ...
        $buffer = array_merge(
            $buffer,
            unpack('C*', pack('v', sizeof($this->privileges))),
        );
...
class AccessToken
{
    ...
    static function init($appID, $appCertificate, $channelName, $uid)
    {
        ...
        if (
            ...
            !$accessToken->is_nonempty_string(
                'appCertificate',
                $appCertificate,
            ) ||
    ...
    function extract($token, $appCertificate, $channelName, $uid)
    {
        ...
        $content = base64_decode(
            substr(
                $token,
                $ver_len + $appid_len,
                strlen($token) - ($ver_len + $appid_len),
            ),
        );
    ...
    function build()
    {
        ...
        $ret =
            $version .
            $this->appID .
            base64_encode(implode(array_map('chr', $content)));
        return $ret;
    }
}
